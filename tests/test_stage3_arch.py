...
class Stage3ArchTests(unittest.TestCase):
    ...
    def test_non_schedule_message_does_not_override_cause_tag(self):
        df = pd.DataFrame(
            {
                "주문번호": ["NS-001"],
                "주문유형": ["정상"],
                ...
                "상담메세지": ["전자동의 완료, 특별판매 일시불"],
            }
        )
        pack = build_exception_pack(df, {"yesterday": pd.Timestamp("2026-03-12").date(), "m_key": "2026-03"})
        q = pack.get("queue", pd.DataFrame())
        self.assertTrue((q["지연원인(메세지추정)"] == "일정무관(전자서명)").any())
        self.assertFalse((q["원인태그"] == "일정무관(전자서명)").any())

    def test_star_history_note_is_non_schedule(self):
        df = pd.DataFrame(
            {
                "주문번호": ["STAR-001"],
                "주문유형": ["정상"],
    ...
    def test_down_service_note_is_non_schedule(self):
        df = pd.DataFrame(
            {
                "주문번호": ["DS-001"],
                "주문유형": ["정상"],
                ...
                "주소": ["서울 강남구 테헤란로 1"],
                "수취인연락처": ["010-1234-5678"],
                "상담메세지": ["기존제품 내림서비스, 밖으로만내어놔주세요 / 폐기"],
            }
        )
        pack = build_exception_pack(df, {"yesterday": pd.Timestamp("2026-03-12").date(), "m_key": "2026-03"})
        q = pack.get("queue", pd.DataFrame())
        self.assertTrue((q["지연원인(메세지추정)"] == "일정무관(시공서비스)").any())
        self.assertFalse((q["원인태그"] == "일정무관(시공서비스)").any())
