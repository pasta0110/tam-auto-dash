...
if order_df is not None and delivery_df is not None:
    ...
    with tab1:
        ...
        with col4:
            ...
            for p_num in ['01', '05']:
                ...
                p_calc_rows.append({'품목': f'판넬{p_num}', '정상 출고': d_qty, '반품 완료': r_qty, '최종 정산': d_qty - r_qty})
    ...
    # --- TAB 2~4: 기존 로직 유지 ---
    # (지면 관계상 TAB 2, 3, 4의 차트 로직은 이전과 동일하므로 생략하거나 기존 코드를 그대로 사용하시면 됩니다.)
    # [TAB 2, 3, 4 코드 구간...]
    with tab2:
        ...
        if len(all_months) > 0:
            if 'p_idx' not in st.session_state: st.session_state.p_idx = max(0, len(all_months) - 3)
            # ... (이하 기존 차트 코드 생략)

    # --- TAB 5: 배송 지도 (미완성 시나리오 대응) ---
    with tab5:
        st.title("📍 5. 실시간 배송 분포 지도")
        st.info("배송지 주소 데이터를 기반으로 한 시각화 페이지입니다. (준비 중)")

    st.caption(f"최종 업데이트: {yesterday_str} | 영업일 기준: 월~토(공휴일 제외) | 배송상태 '완료(4)' 집계 | 사용자: 탬버쌤")