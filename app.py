...
with tab1:
    ...
    with col1:
        ...
        # 수량 컬럼이 확실히 있는지 확인 (안전장치)
        if '수량' not in order_df.columns: order_df['수량'] = order_df['상품명'].apply(get_qty)
    ...
    with col2:
        ...
        # 수량 컬럼 확인
        if '수량' not in delivery_df.columns: delivery_df['수량'] = delivery_df['상품명'].apply(get_qty)
