...
def _infer_reason_from_message(message: str) -> str:
    ...
    # 날짜형 일정 메모 분기
    # 예: "1/9일", "10.2일", "10.2배송" -> 고객협의
    # 예: "1/15 출고후 연기", "10.2 취소 후 재진행" -> 고객협의(일정변경)
    date_like = bool(re.search(r"\b\d{1,2}[\/\.]\d{1,2}(?:일|배송|설치|방문)?", m))
    if date_like:
        if _hit(["출고후", "출고 후", "연기", "취소", "변경", "상태변경", "매변"]):
            return "고객협의(일정변경)"
        return "고객협의"

    rules = [
        ("일정무관(시공서비스)", ["기존제품 내림서비스", "내림서비스", "기존내림", "기존제품 내림", "내림", "버리는 서비스", "버림", "폐기", "밖으로만내어놔주세요", "밖으로만 내어놔주세요"]),
